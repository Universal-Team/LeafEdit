...
{
	"ScriptInfo": {
		"Name": "Wallet Modifier",
		"Game": "ACWW",
		"Author": "StackZ",
		"Region": "EUR",
		"Description": "Let's you edit your Wallet's Amount.",
		"Version": 1
	},
	"ScriptContent": [{
		"info": {
			"title": "Player 1 Modifier",
			"description": "Modifies Player 1's Wallet Amount."
		},
		"script": [{
			...
		}]
...
bool ScriptHelper::isCorrectVersion(const std::vector<ScriptInfo> &info, int entry) {
	switch(save->getType()) {
		...
		case SaveType::NL:
			if (std::find(info[entry].Games.begin(), info[entry].Games.end(), "NL") != info[entry].Games.end()) return true;
			else return false;
		case SaveType::WA:
			if (std::find(info[entry].Games.begin(), info[entry].Games.end(), "WA") != info[entry].Games.end()) return true;
			else return false;
...
bool ScriptHelper::checkVersion(const nlohmann::json &info, int entry) {
	...
	switch(save->getType()) {
		...
		case SaveType::NL:
			if (std::find(Games.begin(), Games.end(), "NL") != Games.end()) return true;
			else return false;
		case SaveType::WA:
			if (std::find(Games.begin(), Games.end(), "WA") != Games.end()) return true;
			else return false;
		case SaveType::UNUSED:
			return false;
	}
...
// Check if Region is correct for the Script usage. Do not execute if not. Right now this causes assert errors.
bool ScriptHelper::isCorrectRegion(const std::vector<ScriptInfo> &info, int entry) {
	if (save->getType() == SaveType::WW) {
		...
		// AC:NL & AC:WA are valid cause all region the same.
		return true;
	}
	return false;
}
