...
void PlayerEditor::DrawSubMenu(void) const {
	...
	for (int i = 0; i < 4; i++) {
		if (i == selectedPlayer) {
			Gui::drawAnimatedSelector((float)(100 * i) + 18.f, 45.f, 64, 104, .030f, SELECTED_COLOR, C2D_Color32(0, 0, 0, 0));
		}
	}
...
void PlayerEditor::DrawMainEditor(void) const {
	...
	GFX::DrawSprite(sprites_back_idx, mainButtons[3].x, mainButtons[3].y);
