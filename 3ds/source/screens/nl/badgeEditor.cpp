...
void BadgeEditor::Draw(void) const {
	...
	GFX::DrawNPC(npc_phineas_idx, 30, 45);
	Gui::DrawStringCentered(0, 215, 0.8f, WHITE, Lang::get("CURRENT_BADGE") + g_badges[selectedBadge], 390);
	GFX::DrawBottom();
	DrawBadges();
	GFX::DrawSprite(sprites_pointer_idx, badgeTouch[selectedBadge].x+20, badgeTouch[selectedBadge].y+20);
}
...
u8 BadgeEditor::setAll() {
	...
	while(1)
	{
		...
		GFX::DrawSprite(sprites_pointer_idx, setPos[selection].x+100, setPos[selection].y+15);
		C3D_FrameEnd(0);
