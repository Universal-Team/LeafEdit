...
void TitleSelection::DrawGameSelector(void) const
{
	...
	for (int i = 0; i < 4; i++) {
		...
		if (selectedGame == i) {
			Gui::drawAnimatedSelector(gameButtons[i].x, gameButtons[i].y, gameButtons[i].w, gameButtons[i].h, .030f, TRANSPARENT, SELECTED_COLOR);
		}
	}

	GFX::DrawSprite(sprites_Icon_idx, 16, 90);
	GFX::DrawSprite(sprites_IconWA_idx, 96, 90);
	GFX::DrawSprite(sprites_IconWL_idx, 176, 90);
	GFX::DrawSprite(sprites_IconWW_idx, 256, 90);
...
void TitleSelection::DrawVersionSelector(void) const
{
	...
	if (selectedGame == 0) {
		GFX::DrawSprite(sprites_Icon_idx, 37.5, 80);
	} else if (selectedGame == 1) {
		GFX::DrawSprite(sprites_IconWA_idx, 37.5, 80);
	}
...
void TitleSelection::DrawRegionSelector(void) const {
	...
	if (selectedVersion == 0) {
		GFX::DrawSprite(sprites_gameCard_idx, 140, 80);
	} else if (selectedVersion == 1) {
		GFX::DrawSprite(sprites_sdCard_idx, 140, 80);
	}
