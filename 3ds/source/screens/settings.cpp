...
void Settings::Draw(void) const
{
	if(screenMode == 0) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (Selection == i) {
				GFX::DrawSprite(sprites_pointer_idx, settingsButtons[i].x+130, settingsButtons[i].y+25);
			}
		}
...
void Settings::DrawLangScreen(void) const {
	...
	for (int i = 0; i < 8; i++) {
		...
		if (Config::lang == i) {
			Gui::drawAnimatedSelector(langBlocks[i].x, langBlocks[i].y, langBlocks[i].w, langBlocks[i].h, .030f, TRANSPARENT, SELECTED_COLOR);
		}
	}
	...
	GFX::DrawSprite(sprites_back_idx, langBlocks[8].x, langBlocks[8].y);
}
